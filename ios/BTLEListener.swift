...
//  Rakning
//
//  Created by Floater on 12.03.20.
...
//            central.scanForPeripherals(withServices: [BTLEBroadcaster.primaryServiceUUID], options: [CBCentralManagerScanOptionAllowDuplicatesKey: true])
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        if let data = characteristic.value {
            ...
            let uuidString = String(data: data, encoding: String.Encoding.ascii)!
            print("  UUID STRING: \(uuidString)")
            
            if let val = characteristic.value {
                let fval = String(describing: val)
                print("    CHAR VALUE: peripheral id: " + peripheral.identifier.uuidString + " uuid: " + uuidString + " characteristic: " + uid + " value: " + fval)
            }
            let pid = peripheral.identifier.uuidString
            if (lastRssi.keys.contains(pid)) {
                let rssi = Int(truncating: lastRssi[pid]!)
                print("    Adding distance for deviceID: " + uuidString + " rssi: \(rssi)")
            }
...
//            }
    ...
    func doSomethingWithIdentityWeFound(data: Data) {
        let uuidString = String(data: data, encoding: String.Encoding.ascii)!
        let contactEvent = ContactEvent(uuid: uuidString)
